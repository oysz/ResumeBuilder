...
当你要发布一个新的正式版本时，建议按下面的顺序操作：

```bash
# 1. 修改版本号
# - 根目录 package.json
# - apps/desktop/package.json
# - 如需同步移动端版本，可修改 apps/mobile/app.json
...
- `v*` tag 会触发 `.github/workflows/release.yml`
- Electron 会直接打包并上传 GitHub Releases
- 若配置了 `EXPO_TOKEN`，RN 会额外触发 Android / iOS 的 EAS 构建
...
引入 `apps/mobile` 后，`v*` tag 仍然可以作为统一发布入口，但移动端不再直接产出 GitHub Release 资产，而是通过 **EAS Build** 生成 Android / iOS 生产构建。

- Android：建议生成 `aab`，用于 Google Play 上架
- iOS：建议通过 EAS 构建后再提交到 TestFlight / App Store
- 需要在仓库配置 `EXPO_TOKEN`，并保持 `apps/mobile/eas.json` 可用
- 如果未配置 `EXPO_TOKEN`，现有 Electron Release 仍会照常执行，不受影响
